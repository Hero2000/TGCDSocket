...
@interface HHSocketResponseFormatter ()

@property (strong, nonatomic) NSData *responseData;

@end

@implementation HHSocketResponseFormatter

+ (instancetype)formatterWithResponseData:(NSData *)data {
    
    HHSocketResponseFormatter *formatter = [HHSocketResponseFormatter new];
...
+ (int)responseContentLengthFromData:(NSData *)data {
    return [HHDataFormatter msgContentLengthFromData:[data subdataWithRange:NSMakeRange(MsgResponsePrefixLength - MsgContentLength, MsgContentLength)]];
}

+ (NSData *)responseAdlerFromData:(NSData *)data {
    return [data subdataWithRange:NSMakeRange(MsgResponsePrefixLength + [self responseContentLengthFromData:data], Adler32Length)];
}

+ (NSData *)responseContentFromData:(NSData *)data {
    return [data subdataWithRange:NSMakeRange(MsgResponsePrefixLength, [self responseContentLengthFromData:data])];
}

- (int)responseType {
    return [HHSocketResponseFormatter responseTypeFromData:self.responseData];
}

- (int)responseCode {
    return [HHSocketResponseFormatter responseCodeFromData:self.responseData];
}

- (int)responseTypeTail {
    return [HHSocketResponseFormatter responseTypeTailFromData:self.responseData];
}

- (int)responseSerialNumber {
    return [HHSocketResponseFormatter responseSerialNumberFromData:self.responseData];
}

- (int)responseContentLength {
    return [HHSocketResponseFormatter responseContentLengthFromData:self.responseData];
}

- (NSData *)responseAdler {
    return [HHSocketResponseFormatter responseAdlerFromData:self.responseData];
}

- (NSData *)responseContent {
    return [HHSocketResponseFormatter responseContentFromData:self.responseData];
}
